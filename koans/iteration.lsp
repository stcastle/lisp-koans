...
(define-test test-dolist
  ...
  (let ((how-many-in-list 0)
    ...
    (assert-equal (car (last some-primes)) biggest-in-list))
  (let ((sum 0))
...
(define-test test-mapcar-with-defun
  ...
  (let* ((subjects '("Astronomy" "Biology" "Chemistry" "Linguistics"))
    ...
    ;; I'm adding here because I don't want to manually replace the x's.
    (let ((manual-result))
      (dolist (subject subjects manual-result)
	(push (vowels-to-xs subject) manual-result))
      (assert-equal mc-result (nreverse manual-result)))))
